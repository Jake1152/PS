...
def l_rotate(dx, dy):
    dx = dx[direc:]  + dx[:direc]
    dy = dy[direc:]  + dy[:direc]

    return dx, dy

def clean(location, rec):
    global cnt
    global direc
    ...
    for i in range(4):
        ...
        print(f"{x}, {y}")
        if rec[x][y] == 0:
            return clean((x, y), rec)
        # 두번째 뒤로갈때 재귀가 보던 방향에서 가기 떄문에
        # 왔던길대로 가는게 아니라서 문제이다.
    r_x = r - dx[-1]
    r_y = c - dy[-1]

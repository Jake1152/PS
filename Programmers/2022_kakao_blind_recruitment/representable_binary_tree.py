...
- 노드들 가장 왼쪽노드부터 가장 오늘쪽 노드까지 왼졲에 있는 순서대로 살펴봄
...
def solution(numbers):
    ...
    def check_nonterminal_node(binary_number, start, end):
        # if length < 0:
        #     return False
        ...
        if (binary_number[parent_idx] == '1'):
            return check_nonterminal_node(binary_number, start, parent_idx) and \
                    check_nonterminal_node(binary_number, parent_idx + 1, end)
        else:
            return False
    ...
    for number in numbers:
        ...
        if check_nonterminal_node(binary, 0, len(binary)):
            answer.append(1)
        else:
            answer.append(0)
        print()
...
test_case.append([64, 63, 42, 255, 511, 1023, 2047, 4095, 8191, 2**15-1])
